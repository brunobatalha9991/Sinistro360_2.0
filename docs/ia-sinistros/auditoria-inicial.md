...
1. ~~**Timezone oficial**~~ — **Respondido (2026-08-22): `America/Sao_Paulo`, fixo.** Já implementado em `logic/responsabilidade.js` (`TIMEZONE_OFICIAL`).
2. ~~**Definição de "responsável no dia"**~~ — **Respondido: responsável no momento da ação** (resolução por intervalo exato, não por dia predominante nem proporção de horas). Já implementado (`responsavelVigenteEm()`).
3. **Equipe/gestor/carteira/filial** — **Respondido (2026-08-22)**: não existe uma entidade separada — é derivado do `role` já existente: `admin`/`atendente` atuam como **gestor**, `analista`/`consulta` são a **equipe** gerida. Cada usuário `consulta` tem uma carteira de clientes própria na operação real, mas **isso ainda não está representado no modelo de dados** (nenhum campo liga um cliente/segurado a um usuário `consulta` específico) — é uma lacuna real a resolver antes da Fase 5 poder filtrar por carteira. Não há hierarquia N:N (um gestor específico para um usuário específico), só a divisão binária por `role`.
4. **SLA formal** — **Respondido: não existe.** O campo `nextAction.date` ("Próxima ação", definido manualmente) já é o controle de prazo real usado pela operação — a Fase 5 deve usar exatamente essa regra (`isAtrasado()`), sem inventar um SLA formal por etapa/seguradora que não existe na prática.
5. ~~**Apetite por backend mínimo**~~ — **Respondido: recusado — sem custo de infraestrutura nova.** Projeto permanece 100% frontend; risco aceito documentado em `regras-responsabilidade.md` (seção "Risco aceito").
6. **Confirmação do "banco de dados"** — ainda não confirmado explicitamente pelo usuário, mas nenhuma indicação em contrário até aqui; segue como premissa de trabalho (Firestore, sem plano de banco relacional).
7. ~~**Backfill do histórico de responsabilidade**~~ — **Respondido: reconstruir parcialmente, marcado como "estimado".** Já implementado (`estimarHistoricoLegado()`).
8. **Escala real da operação** — **Respondido: até 5 usuários ativos.** Confirma que a robustez completa do Painel de Desempenho (filtros por equipe/gestor/carteira, drill-down, alertas de qualidade de dados) pode esperar — nesse porte, um painel simples por usuário/período já cobre a necessidade real; a granularidade completa só se paga se a operação crescer.
9. **Multi-tenant** — **Respondido: uso interno de uma única corretora.** Confirmado — não é necessário desenhar isolamento de dados entre organizações.
10. **Dados sensíveis (LGPD)**: ainda em aberto — precisa confirmação de quem administra a integração CORP sobre se a API real envia CPF/CNPJ/dados bancários não mapeados por `mapCorp()`.

**Ainda bloqueiam decisões de desenho antes da Fase 5**: pergunta 10 (LGPD) e a lacuna de "carteira por usuário `consulta`" identificada na resposta da pergunta 3 (precisa de um novo campo — ex. vincular `corp_users.carteira` a um critério de segurado/cliente — antes de qualquer filtro de carteira funcionar de verdade).
