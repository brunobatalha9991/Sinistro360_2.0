...
export const searchClaimsTool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      situacao: { type: "STRING", description: "Filtra pela situação efetiva, ex.: 'Em andamento', 'Indenizado', 'Pendente', 'Encerrado sem Indenização'." },
  ...
  run(args, ctx) {
    ...
    if (situacaoFiltro) {
      claims = claims.filter((c) => situacaoEfetiva(overrides, c).label.toLowerCase().indexOf(situacaoFiltro) >= 0);
    }
    ...
    const resultados = claims.slice(0, limit).map((c) => ({
      ...
      situacao: situacaoEfetiva(overrides, c).label,
