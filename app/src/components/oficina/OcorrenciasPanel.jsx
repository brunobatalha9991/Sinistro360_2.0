...
// Reclamações e feedbacks da oficina (corp_oficina_ocorrencias), com
// vínculo opcional a um processo existente, filtro De/Até e exportação em
// CSV do período filtrado — a pedido do usuário.
export function OcorrenciasPanel({ oficinaId, claims, actions, canEdit, navigate }) {
  ...
  const todas = actions.ocorrencias.filter((o) => o.oficinaId === oficinaId);
  ...
  function registrar() {
    ...
    actions.addOcorrencia(oficinaId, { tipo, titulo, descricao, data, claimId: claimId || null });
  ...
  function exportarCsv() {
    ...
    downloadCSV(`ocorrencias_oficina_${oficinaId}.csv`, rows);
  }
