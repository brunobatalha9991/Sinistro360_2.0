...
// Tarefas vinculadas a esta oficina (t.oficinaId) — mesmo mecanismo de
// prefill já usado em "Criar tarefa vinculada a este processo"
// (DetailHeader.jsx), agora com oficinaId em vez de processoId.
export function TarefasOficinaPanel({ oficinaId, oficinaNome, navigate }) {
  const { records } = useData();
  const tarefas = (records.corp_tasks || []).filter((t) => t.oficinaId === oficinaId);
    ...
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        ...
        <button
          ...
          onClick={() => {
            setDemandaPrefill({ titulo: `Oficina ${oficinaNome}`, descricao: "", oficinaId });
            navigate("tarefas", "newfromdemanda");
          }}
        >
          + Criar tarefa vinculada a esta oficina
        </button>
      </div>
      ...
      {tarefas.length ? (
        ...
        <div style={{ marginTop: 14 }}><EmptyState>Nenhuma tarefa vinculada a esta oficina ainda.</EmptyState></div>
      )}
    </div>
  );
}
