...
// Comunicação do gestor com a oficina (a pedido do usuário) — registro
// independente do histórico de cada sinistro, pensado como evidência pra
// alinhar em reuniões com a oficina.
export function ComunicacaoGestorPanel({ oficinaId, actions, canEdit }) {
  const [tipo, setTipo] = useState(TIPOS[0]);
  const [data, setData] = useState(todayISO());
  const [resumo, setResumo] = useState("");

  const registros = actions.comunicacoes
    .filter((x) => x.oficinaId === oficinaId)
  ...
  function registrar() {
    ...
    actions.addComunicacaoGestor(oficinaId, { tipo, data, resumo });
    setResumo("");
  }

  return (
    <div className="card">
      <h3 style={{ marginTop: 0 }}>Registrar comunicação com a oficina</h3>
