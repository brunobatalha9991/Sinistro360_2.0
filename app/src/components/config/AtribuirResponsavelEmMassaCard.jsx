...
function BlocoStatus({ status, claims, overrides, users, actions, canEdit }) {
  const [usuarioId, setUsuarioId] = useState("");
  const [resultado, setResultado] = useState(null);

  const semResponsavel = useMemo(() => claims.filter((c) => (
    situacaoEfetiva(overrides, c).label === status && !getResponsavel(overrides, c.id)
  )), [claims, overrides, status]);
...
export function AtribuirResponsavelEmMassaCard({ claims, overrides, users, actions, canEdit }) {
  return (
    <div className="card">
      ...
      <BlocoStatus status="Pendente" claims={claims} overrides={overrides} users={users} actions={actions} canEdit={canEdit} />
      <BlocoStatus status="Em andamento" claims={claims} overrides={overrides} users={users} actions={actions} canEdit={canEdit} />
    </div>
  );
}
