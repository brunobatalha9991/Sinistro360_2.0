...
export function UsersCard({ users, currentUser, saveRecord }) {
  ...
  function criarUsuario() {
    ...
    saveUsers((current) => [...(current || []), { id: uid("usr"), nome: n, email: e, senha: s, role, modulos }]);
  ...
  function redefinirSenha(u) {
    ...
    saveUsers((current) => (current || []).map((x) => (x.id === u.id ? { ...x, senha: trimmed } : x)));
  ...
  function editar(u) {
    ...
    const ns = prompt("Senha:", u.senha); if (ns === null) return;
    ...
    saveUsers((current) => (current || []).map((x) => (x.id === u.id ? { id: u.id, nome: nn.trim(), email: ne.trim(), senha: ns, role: nr, modulos: u.modulos } : x)));
  }
