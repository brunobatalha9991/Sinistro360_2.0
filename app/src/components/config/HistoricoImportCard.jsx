...
// do HTML original.
export function HistoricoImportCard({ claims, actions, canEdit }) {
  ...
  function importFile(file) {
    ...
    reader.onload = (e) => {
      const result = processHistoricoCsv(e.target.result, claims, currentUser?.nome, (claimId, comment) => {
      ...
      alert(`Importação concluída: ${result.ok} registrado(s), ${result.falha} com problema. O relatório de validação foi baixado.`);
    };
  ...
  return (
    <div className="card">
      ...
      <p className="muted">
        Baixe a planilha modelo, preencha a coluna "Últ. Histórico" no formato DD/MM/AAAA: descrição (ex.: 10/08/2026: Aguard. chegada de peças) e importe de volta. O sistema localiza o processo pelo Nosso N°, Tipo, Segurado/Terceiro e N° Sinistro, e registra a comunicação com Com quem = Dados importado e Meio = Dados importado.
      </p>
