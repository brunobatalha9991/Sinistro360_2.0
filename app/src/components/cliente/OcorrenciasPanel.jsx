...
// Reclamações e feedbacks do cliente (corp_cliente_ocorrencias), com
// vínculo opcional a um processo existente, filtro De/Até e exportação em
// CSV do período filtrado — a pedido do usuário.
export function OcorrenciasPanel({ clienteId, claims, actions, canEdit, navigate }) {
  ...
  const todas = actions.ocorrencias.filter((o) => o.clienteId === clienteId);
  ...
  function registrar() {
    ...
    actions.addOcorrencia(clienteId, { tipo, titulo, descricao, data, claimId: claimId || null });
  ...
  function exportarCsv() {
    ...
    downloadCSV(`ocorrencias_cliente_${clienteId}.csv`, rows);
  }
