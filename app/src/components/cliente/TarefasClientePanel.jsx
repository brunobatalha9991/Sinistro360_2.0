...
// Tarefas vinculadas a este cliente (t.clienteId) — mesmo mecanismo de
// prefill já usado em "Criar tarefa vinculada a este processo"
// (DetailHeader.jsx), agora com clienteId em vez de processoId.
export function TarefasClientePanel({ clienteId, clienteNome, navigate }) {
  const { records } = useData();
  const tarefas = (records.corp_tasks || []).filter((t) => t.clienteId === clienteId);
    ...
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        ...
        <button
          ...
          onClick={() => {
            setDemandaPrefill({ titulo: `Cliente ${clienteNome}`, descricao: "", clienteId });
            navigate("tarefas", "newfromdemanda");
          }}
        >
          + Criar tarefa vinculada a este cliente
        </button>
      </div>
      ...
      {tarefas.length ? (
        ...
        <div style={{ marginTop: 14 }}><EmptyState>Nenhuma tarefa vinculada a este cliente ainda.</EmptyState></div>
      )}
    </div>
  );
}
