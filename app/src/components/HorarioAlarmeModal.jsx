...
// deixa de estar vencido, ex.: a etapa foi concluída). Usa as classes
// genéricas ".alarme-geral-*" (ver global.css), compartilhadas com
// TarefaAlarmeModal.jsx.
export function HorarioAlarmeModal({ alarmes, onDismiss, onDismissAll, navigate }) {
  ...
  return createPortal(
    <div className="alarme-geral-overlay" style={{ "--alarm-rgb": "var(--danger-rgb)" }}>
      <div className="alarme-geral-box">
        <div className="alarme-geral-icone">🚨</div>
        <h2 className="alarme-geral-titulo">HORÁRIO VENCIDO</h2>
        ...
        <div className="alarme-geral-lista">
          {alarmes.map((a) => (
            <div key={a.key} className="alarme-geral-item">
              <div>
                <div className="alarme-geral-item-titulo">{a.numsin ? `Sinistro ${a.numsin}` : "Processo"} — {a.segurado || "—"}</div>
                <div className="alarme-geral-item-sub">{a.title} • {a.label}: {fmtDateBR(a.date)} às {a.hora}</div>
              </div>
