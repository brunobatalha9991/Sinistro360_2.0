...
export function getAllCols({ overrides, allClaimsRaw, navigate, atendTemplateCfg, templates }) {
  return [
    ...
    { key: "ramo", label: "Ramo", cell: (c) => txt(campoEfetivo(overrides, c, "ramo")), sortValue: (c) => campoEfetivo(overrides, c, "ramo") || "" },
    {
      ...
      cell: (c) => { const s = situacaoEfetiva(overrides, c, atendTemplateCfg, templates); return <span className={"badge " + s.cls}>{s.label}</span>; },
      sortValue: (c) => situacaoEfetiva(overrides, c, atendTemplateCfg, templates).label || "",
    },
    {
